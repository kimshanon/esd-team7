...
PAYMENT_SERVICE_URL = "http://localhost:5008" # Calculate Payment microservice URL
...
def cancel_order(order_id):
    ...
    try:
        ...
        # Only process cancellation if order is Active
        if order_status == "Active":
            # Get payment ID from order data
            payment_id = order_data.get("payment_id")
            if payment_id:
                # Process refund through calculate_payment service
                refund_response = requests.post(
                    f"{PAYMENT_SERVICE_URL}/api/v1/orders/{payment_id}/refund"
                )
                
                if refund_response.status_code != 200:
                    return jsonify({
                        "code": refund_response.status_code,
                ...
                return jsonify({
                    "code": 400,
                    "message": "No payment ID found for order",
                    "data": None
                }), 400
        else:
            return jsonify({
...
def handle_order_cancelled(message):
    ...
    try:
        ...
        if order_status == "Active":
            ...
            # If order is Active, trigger refund process
            payment_id = order_data.get("payment_id")
            if payment_id:
                refund_response = requests.post(
                    f"{PAYMENT_SERVICE_URL}/api/orders/{payment_id}/refund"
                )
                if refund_response.status_code != 200:
...
def create_order():
    ...
    try:
        ...
        print(f"Sending to order service: {ORDER_SERVICE_URL}/api/v1/orders")
        ...
        # Process payment through calculate_payment service
        payment_data = {
        ...
        # Store the order data in the pending order dict
...
# =========================================================================
# 9. Endpoint to get pending orders for picker to see
# =========================================================================
...
# =========================================================================
# 10. Endpoint to get active pickers (Not that necessary)
# =========================================================================
...
# =========================================================================
# 11. Websocket endpoints for UI to interact with
# =========================================================================
@socketio.on("connect")
def handle_connect():
...
# =========================================================================
# 12. Register Picker and Customer and Test
# =========================================================================
